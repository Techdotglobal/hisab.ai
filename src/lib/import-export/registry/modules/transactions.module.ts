...
function makeModule(c: Config): ModuleDefinition {
  ...
  return {
    ...
    async createRecord(record, ctx) {
      ...
      try {
        if(c.lineTable && r.lines.length) { const lineRows=[]; for (const [lineIndex,l] of (r.lines as any[]).entries()) { if(String(l.detailType)==='SubTotalLineDetail')continue; const accountLink=l.accountSourceId&&realmId?await resolveQuickBooksLocalId(ctx.companyId,realmId,String(l.accountSourceId),['Account'],['chart_of_accounts']):null; const itemLink=l.itemSourceId&&realmId?await resolveQuickBooksLocalId(ctx.companyId,realmId,String(l.itemSourceId),['Item'],['inventory_items']):null; const classLink=l.classSourceId&&realmId?await resolveQuickBooksLocalId(ctx.companyId,realmId,String(l.classSourceId),['Class'],['cost_centers']):null; const aid=c.kind==='vendorCredit'?await resolveVendorCreditLineAccount(ctx.companyId,realmId,l,itemLink?.id):c.kind==='salesReceipt'?await resolveSalesReceiptLineAccount(ctx.companyId,realmId,l):accountLink?.id??await accountId(ctx.companyId,l.accountNo); if(['journal','salesReceipt'].includes(c.kind)&&!aid)throw new Error(`QuickBooks ${c.displayName} line ${lineIndex+1} has no migrated account.`); const rawLineAmount=Number(l.amount??0); const rawUnitPrice=Number(l.unitPrice??l.amount??0); const line:any={[c.lineForeignKey!]:data.id,company_id:ctx.companyId,account_id:aid ?? null,description:l.description ?? r.description ?? `Imported QuickBooks ${c.kind} line`,quantity:l.quantity ?? 1,unit_price:c.kind==='bill'?Math.abs(rawUnitPrice):l.unitPrice ?? l.amount ?? 0,amount:c.kind==='bill'?Math.abs(rawLineAmount):l.amount ?? 0,tax_rate:Number.isFinite(Number(l.taxRate))?Number(l.taxRate):r.subtotal>0?(r.taxAmount/r.subtotal)*100:0}; if(c.kind==='bill') line.is_reduction=rawLineAmount<0; if(c.kind==='expense'){delete line.quantity;delete line.unit_price} if(['bill','vendorCredit','salesReceipt'].includes(c.kind)) line.inventory_item_id=itemLink?.id??null; if(['bill','journal','vendorCredit','salesReceipt'].includes(c.kind)) line.cost_center_id=classLink?.id??null; if(['vendorCredit','salesReceipt'].includes(c.kind))Object.assign(line,{line_no:lineIndex+1,source_line_id:l.sourceLineId??null,detail_type:l.detailType??(c.kind==='salesReceipt'?'SalesItemLineDetail':'AccountBasedExpenseLineDetail'),source_account_id:l.accountSourceId??null,source_item_id:l.itemSourceId??null,source_class_id:l.classSourceId??null,source_tax_code_id:l.taxCodeSourceId??null,metadata:{raw:l}}); if(c.kind==='journal') { delete line.quantity; delete line.unit_price; delete line.amount; line.debit=l.debit??0; line.credit=l.credit??0 } lineRows.push(line) } if(c.kind==='bill')for(const line of lineRows){if(Number(line.amount)<0||Number(line.unit_price)<0)throw new Error(`QuickBooks ${c.displayName} produced a negative amount/unit_price line, which violates the bill_lines_amount_nonneg_chk / bill_lines_unit_price_nonneg_chk constraints.`)} const {error:le}=await createAdminClient().from(c.lineTable).insert(lineRows); if(le) throw le }
