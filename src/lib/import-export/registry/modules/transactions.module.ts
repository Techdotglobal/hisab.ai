...
import { createOrUpdateVendorOpenItem } from '@/lib/accounting/vendor-open-items'
...
const fields: FieldDefinition[] = [
  ...
  { key: 'settlementAccountSourceId', label: 'Settlement Account QuickBooks ID', type: 'string' },
...
function parseRow(m: Record<string, unknown>) {
  ...
  return { ...m, sourceId:text(m.sourceId), transactionNo, date:text(m.date), dueDate:text(m.dueDate), expiryDate:text(m.expiryDate), expectedDate:text(m.expectedDate), customerName:text(m.customerName), customerSourceId:text(m.customerSourceId), vendorName:text(m.vendorName), vendorSourceId:text(m.vendorSourceId), amount:parseNumberField(m.amount,total), subtotal:parseNumberField(m.subtotal,total), taxAmount:parseNumberField(m.taxAmount,0), total, status:text(m.status), currency:(text(m.currency) ?? 'USD').toUpperCase(), exchangeRate:parseNumberField(m.exchangeRate,1), homeTotal:parseNumberField(m.homeTotal,total*parseNumberField(m.exchangeRate,1)), reference:text(m.reference), description:text(m.description) ?? `Imported QuickBooks transaction ${transactionNo}`, category:text(m.category), paymentMethod:text(m.paymentMethod),paymentMethodSourceId:text(m.paymentMethodSourceId),depositAccountSourceId:text(m.depositAccountSourceId),apAccountSourceId:text(m.apAccountSourceId),settlementAccountSourceId:text(m.settlementAccountSourceId), invoiceNo:text(m.invoiceNo), billNo:text(m.billNo), relatedSourceId:text(m.relatedSourceId), allocations,unappliedAmount:parseNumberField(m.unappliedAmount,0),relationshipIssues,lines,sourcePayloadHash:typeof m._quickbooksRaw==='string'?createHash('sha256').update(m._quickbooksRaw).digest('hex'):undefined }
}

async function isAccountsPayableAccount(companyId:string,realmId:string,accountSourceId:string){
  const archived=await createAdminClient().from('quickbooks_migration_records').select('source_payload').eq('company_id',companyId).eq('realm_id',realmId).eq('entity_type','Account').eq('source_id',accountSourceId).maybeSingle()
  if(archived.error)throw archived.error
  return String((archived.data?.source_payload as Record<string,unknown>|undefined)?.AccountType??'')==='Accounts Payable'
}

/**
 * A JournalEntryLineDetail line that posts to an Accounts-Payable account and names a vendor represents a vendor
 * liability (Credit) or a vendor-credit-like reduction (Debit) that today has no representation outside the general
 * ledger — see migration 075_vendor_open_items.sql. The JE itself is and remains the only ledger posting; this only
 * indexes it for AP aging/vendor statements/payment allocation. Uses the ORIGINAL source lines (before the VAT-line
 * injection above), since a synthetic VAT line never carries a vendor entity.
 */
async function createJournalVendorOpenItems(companyId:string,realmId:string|undefined,r:ReturnType<typeof parseRow>,journalId:string){
  if(!realmId)return
  for(const l of r.lines as any[]){
    if(String(l.entityType)!=='Vendor'||!l.entitySourceId||!l.accountSourceId)continue
    if(!(await isAccountsPayableAccount(companyId,realmId,String(l.accountSourceId))))continue
    const vendor=await resolveQuickBooksLocalId(companyId,realmId,String(l.entitySourceId),['Vendor'],['vendors'])
    if(!vendor)throw new Error(`QuickBooks vendor ${l.entitySourceId} must be migrated before Journal Entry ${r.sourceId} can represent its AP line.`)
    const amount=Number(l.debit??0)>0?Number(l.debit):Number(l.credit??0)
    if(!(amount>0))continue
    await createOrUpdateVendorOpenItem({
      companyId,vendorId:vendor.id,direction:Number(l.credit??0)>0?'PAYABLE':'CREDIT',sourceType:'JOURNAL_ENTRY',sourceId:journalId,
      sourceLineId:l.sourceLineId?String(l.sourceLineId):null,sourceReference:`${r.sourceId}:${l.sourceLineId??''}`,
      date:r.date?new Date(r.date):new Date(),currency:r.currency,exchangeRate:r.exchangeRate,total:amount,
      description:l.description?String(l.description):r.description,
    })
  }
}

/**
 * Generalizes the same mechanism to a QuickBooks expense (Purchase) with an AccountBasedExpenseLineDetail line posting
 * to Accounts Payable — NETKOM: 13 expenses, one vendor, SAR 148,750.00, already correctly posted (Dr AP / Cr
 * settlement account) but invisible to AP aging/vendor statements because `expenses` carries no vendor identity at all.
 * Unlike a journal line, a Purchase line has no per-line entity — the vendor is the expense's own header VendorRef.
 */
async function createExpenseVendorOpenItems(companyId:string,realmId:string|undefined,r:ReturnType<typeof parseRow>,expenseId:string){
  if(!realmId||!r.vendorSourceId)return
  for(const l of r.lines as any[]){
    if(!l.accountSourceId)continue
    if(!(await isAccountsPayableAccount(companyId,realmId,String(l.accountSourceId))))continue
    const vendor=await resolveQuickBooksLocalId(companyId,realmId,r.vendorSourceId,['Vendor'],['vendors'])
    if(!vendor)throw new Error(`QuickBooks vendor ${r.vendorSourceId} must be migrated before Expense ${r.sourceId} can represent its AP line.`)
    const amount=Math.abs(Number(l.amount??0))
    if(!(amount>0))continue
    await createOrUpdateVendorOpenItem({
      companyId,vendorId:vendor.id,direction:'PAYABLE',sourceType:'EXPENSE',sourceId:expenseId,
      sourceLineId:l.sourceLineId?String(l.sourceLineId):null,sourceReference:`${r.sourceId}:${l.sourceLineId??''}`,
      date:r.date?new Date(r.date):new Date(),currency:r.currency,exchangeRate:r.exchangeRate,total:amount,
      description:l.description?String(l.description):r.description,
    })
...
async function preservePaymentAllocations(input:{companyId:string;realmId?:string;paymentId:string;kind:'customerPayment'|'vendorPayment';row:ReturnType<typeof parseRow>;fallbackTargetId?:string;vendorId?:string|null}){
  const {companyId,realmId,paymentId,kind,row,fallbackTargetId,vendorId}=input
  ...
  if(row.allocations.length){if(!realmId||!row.sourceId)throw new Error('QuickBooks payment allocations require realm and source identifiers.');const resolved=await resolveQuickBooksPaymentAllocations({companyId,realmId,sourcePaymentId:row.sourceId,kind:kind==='customerPayment'?'CUSTOMER':'VENDOR',currency:row.currency,exchangeRate:row.exchangeRate,allocations:row.allocations,vendorId});await replacePaymentAllocations(companyId,paymentId,resolved);return}
...
async function paymentAllocationsMatch(companyId:string,paymentId:string,realmId:string|undefined,kind:'customerPayment'|'vendorPayment',row:ReturnType<typeof parseRow>,vendorId?:string|null){
  if(!realmId||!row.sourceId||row.relationshipIssues.length)return false
  const expected=await resolveQuickBooksPaymentAllocations({companyId,realmId,sourcePaymentId:row.sourceId,kind:kind==='customerPayment'?'CUSTOMER':'VENDOR',currency:row.currency,exchangeRate:row.exchangeRate,allocations:row.allocations,vendorId}),db=createAdminClient()
  // deposit_account_id now carries the source-resolved settlement account for both payment directions (see createRecord).
  const expectedDepositAccount=row.depositAccountSourceId?await resolveQuickBooksLocalId(companyId,realmId,row.depositAccountSourceId,['Account'],['chart_of_accounts']):null
  if(row.depositAccountSourceId&&!expectedDepositAccount)return false
  ...
  if(String(payment.data.deposit_account_id??'')!==String(expectedDepositAccount?.id??''))return false
    ...
    async createRecord(record, ctx) {
      ...
      if(c.kind==='invoice') {
        ...
        for(const line of r.lines as any[]) {
          if(['SubTotalLineDetail','DescriptionOnly'].includes(String(line.detailType)))continue
          ...
          // line.taxRate for a QuickBooks-sourced row is a TaxCodeRef ID (e.g. "11"), never a real percentage — it must
          // never be trusted as one. fallbackRate is derived from QuickBooks' own TxnTaxDetail.TotalTax and is authoritative
          // whenever there is a subtotal to derive it from. A CSV/native import (no realmId) has no such header and does
          // supply a genuine percentage in line.taxRate, so that existing behavior is preserved unchanged.
          const taxRate=realmId?(r.subtotal>0?fallbackRate:(Number.isFinite(parsedTax)?parsedTax:0)):(Number.isFinite(parsedTax)?parsedTax:fallbackRate)
          const resolvedAccountId=accountLink?.id??await accountId(ctx.companyId,line.accountNo)??null
          if(realmId&&line.accountSourceId&&!resolvedAccountId)throw new Error(`QuickBooks revenue account ${line.accountSourceId} must be migrated before Invoice ${r.sourceId}.`)
          nativeLines.push({description:String(line.description??r.description??'Imported QuickBooks line'),quantity:Number(line.quantity??1),unitPrice:Number(line.unitPrice??line.amount??0),taxRate,accountId:resolvedAccountId,costCenterId:classLink?.id??customerContext?.projectId??null,projectId:customerContext?.projectId??null,inventoryItemId:itemLink?.id??null})
        }
      ...
      if (c.kind==='expense') {
        const settlementAccount=r.settlementAccountSourceId&&realmId?await resolveQuickBooksLocalId(ctx.companyId,realmId,r.settlementAccountSourceId,['Account'],['chart_of_accounts']):null
        if(r.settlementAccountSourceId&&realmId&&!settlementAccount)throw new Error(`QuickBooks settlement account ${r.settlementAccountSourceId} must be migrated before Expense ${r.sourceId}.`)
        Object.assign(header,{description:r.description || `Imported QuickBooks expense ${r.transactionNo}`,category:r.category ?? 'Other',total:r.total,tax_amount:r.taxAmount,exchange_rate:r.exchangeRate,base_total:r.homeTotal,settlement_account_id:settlementAccount?.id??null})
      }
      ...
      if(c.kind==='customerPayment'||c.kind==='vendorPayment') { header.amount=r.amount; header.method=r.paymentMethod; header.reference=r.reference;header.exchange_rate=r.exchangeRate;header.base_amount=r.homeTotal;header.customer_id=c.kind==='customerPayment'?customerId:null;header.vendor_id=c.kind==='vendorPayment'?vendorId:null;
        // deposit_account_id holds the source-resolved settlement account for BOTH directions: customer payments read it
        // from DepositToAccountRef, vendor payments from CheckPayment.BankAccountRef (both funnelled into the same
        // depositAccountSourceId field by the adapter). postPaymentToLedger already treats it as direction-agnostic.
        if(r.depositAccountSourceId&&realmId){const depositAccount=await resolveQuickBooksLocalId(ctx.companyId,realmId,r.depositAccountSourceId,['Account'],['chart_of_accounts']);if(!depositAccount)throw new Error(`QuickBooks deposit account ${r.depositAccountSourceId} must be migrated before payment ${r.sourceId}.`);header.deposit_account_id=depositAccount.id}
        paymentTargetId=await relatedId(c.kind==='customerPayment'?'invoices':'bills',ctx.companyId,c.kind==='customerPayment'?'invoice_no':'bill_no',c.kind==='customerPayment'?r.invoiceNo:r.billNo,r.relatedSourceId); if(paymentTargetId) { if(c.kind==='customerPayment') header.invoice_id=paymentTargetId; else header.bill_id=paymentTargetId } }
      const {data,error}=await createAdminClient().from(c.table).insert(header).select('id').single(); if(error) throw error
      try {
        if(c.lineTable && r.lines.length) { const lineRows=[]; for (const [lineIndex,l] of (r.lines as any[]).entries()) { if(['SubTotalLineDetail','DescriptionOnly'].includes(String(l.detailType)))continue; const accountLink=l.accountSourceId&&realmId?await resolveQuickBooksLocalId(ctx.companyId,realmId,String(l.accountSourceId),['Account'],['chart_of_accounts']):null; const itemLink=l.itemSourceId&&realmId?await resolveQuickBooksLocalId(ctx.companyId,realmId,String(l.itemSourceId),['Item'],['inventory_items']):null; const classLink=l.classSourceId&&realmId?await resolveQuickBooksLocalId(ctx.companyId,realmId,String(l.classSourceId),['Class'],['cost_centers']):null; const aid=c.kind==='vendorCredit'?await resolveVendorCreditLineAccount(ctx.companyId,realmId,l,itemLink?.id):c.kind==='salesReceipt'?await resolveSalesReceiptLineAccount(ctx.companyId,realmId,l):accountLink?.id??await accountId(ctx.companyId,l.accountNo); if(['journal','salesReceipt'].includes(c.kind)&&!aid)throw new Error(`QuickBooks ${c.displayName} line ${lineIndex+1} has no migrated account.`); const rawLineAmount=Number(l.amount??0); const rawUnitPrice=Number(l.unitPrice??l.amount??0); const normalizesReductionLines=['bill','expense'].includes(c.kind); const line:any={[c.lineForeignKey!]:data.id,company_id:ctx.companyId,account_id:aid ?? null,description:l.description ?? r.description ?? `Imported QuickBooks ${c.kind} line`,quantity:l.quantity ?? 1,unit_price:c.kind==='bill'?Math.abs(rawUnitPrice):l.unitPrice ?? l.amount ?? 0,amount:normalizesReductionLines?Math.abs(rawLineAmount):l.amount ?? 0,tax_rate:Number.isFinite(Number(l.taxRate))?Number(l.taxRate):r.subtotal>0?(r.taxAmount/r.subtotal)*100:0}; if(normalizesReductionLines) line.is_reduction=rawLineAmount<0; if(c.kind==='expense'){delete line.quantity;delete line.unit_price} if(['bill','vendorCredit','salesReceipt'].includes(c.kind)) line.inventory_item_id=itemLink?.id??null; if(['bill','journal','vendorCredit','salesReceipt'].includes(c.kind)) line.cost_center_id=classLink?.id??null; if(['vendorCredit','salesReceipt'].includes(c.kind))Object.assign(line,{line_no:lineIndex+1,source_line_id:l.sourceLineId??null,detail_type:l.detailType??(c.kind==='salesReceipt'?'SalesItemLineDetail':'AccountBasedExpenseLineDetail'),source_account_id:l.accountSourceId??null,source_item_id:l.itemSourceId??null,source_class_id:l.classSourceId??null,source_tax_code_id:l.taxCodeSourceId??null,metadata:{raw:l}}); if(c.kind==='journal') { delete line.quantity; delete line.unit_price; delete line.amount; line.debit=l.debit??0; line.credit=l.credit??0 } lineRows.push(line)
          if(c.kind==='journal'){
            // JournalEntryLineDetail.TaxAmount is an explicit source figure the adapter now preserves (see
            // quickbooks.adapter.ts normalizeTransaction) — QuickBooks records this line's Amount net of tax, so the
            // matching tax posting is added here on the SAME side (debit/credit) as the taxed line, which is exactly
            // what closes the gap that made these journal entries fail their balance check. Never invented: the amount
            // and its direction both come from the source; only the destination VAT account is resolved.
